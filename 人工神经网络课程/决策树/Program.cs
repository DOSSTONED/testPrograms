...
namespace 决策树
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Sample[] Samples = new Sample[]{
                new Sample(Category.第一类,new char[]{'A','E','H','K','M'}),
                new Sample(Category.第一类,new char[]{'B','E','I','L','M'}),
                new Sample(Category.第一类,new char[]{'A','G','I','L','N'}),
                new Sample(Category.第一类,new char[]{'B','G','H','K','M'}),
                new Sample(Category.第一类,new char[]{'A','G','I','L','M'}),
                new Sample(Category.第二类,new char[]{'B','F','I', 'L', 'M'}),
                new Sample(Category.第二类,new char[]{'B','F', 'J', 'L', 'N'}),
                new Sample(Category.第二类,new char[]{'B','E' ,'I', 'L', 'N'}),
                new Sample(Category.第二类,new char[]{'C','G' ,'J', 'K', 'N' }),
                new Sample(Category.第二类,new char[]{'C','G', 'J','L', 'M' }),
                new Sample(Category.第二类,new char[]{'D','G', 'J', 'K', 'M'}),
                new Sample(Category.第二类,new char[]{'B','G', 'I', 'L', 'M'})
            };

            Sample toClassify1 = new Sample(Category.没决定_是因为刚初始化, new char[] { 'B', 'G', 'I', 'K', 'N' });
            Sample toClassify2 = new Sample(Category.没决定_是因为刚初始化, new char[] { 'C', 'E', 'J', 'L', 'M' });
    ...
    class DNode
    {
        ...
        /// 当前类别
        /// </summary>
        public Category currentCategory = Category.没决定_是因为刚初始化;
        /// <summary>
        /// 所有儿子
        /// </summary>
        ...
        public List<DNode> getNext()
        {
            ...
            if ((allSamples.Count(x => x.本样本的类别 == Category.第一类) == allSamples.Length) ||
                 (allSamples.Count(x => x.本样本的类别 == Category.第二类) == allSamples.Length))
            {
                this.currentCategory = allSamples[0].本样本的类别;
                return null;
            }
            /// 没有儿子，说明分类到此为止
