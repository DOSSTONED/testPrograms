...
namespace DES_File_Handle
{
    class DES
    {
        public BitArray Encrypt64bit(BitArray plain, BitArray key)
        {
            return Encrypt(plain, key, true);
        }

        public BitArray Decrypt64bit(BitArray plain, BitArray key)
        {
            return Encrypt(plain, key, false);
        }
        //初始置换表IP  64
        ...
        private BitArray 密钥变换(BitArray key)
        {
        ...
        private BitArray[] 给出16个子密钥(BitArray key56)
        {
            BitArray[] resultprivate = new BitArray[16];
            resultprivate[0] = 循环左移操作(key56, MOVE_TIMES[0]);

            for (int i = 1; i < 16; i++)
                resultprivate[i] = 循环左移操作(resultprivate[i - 1], MOVE_TIMES[i]);
            ...
            //    for (int j = 0; j < 48; j++)
            //    {
            //        ret[i][j] = resultprivate[i][PC_2[j]];
            //    }
            //}
        ...
        private BitArray 循环左移操作(BitArray bits56, int move)
        {
        ...
        private BitArray 从56位密钥变48位子密钥(BitArray key56)
        {
        ...
        private BitArray[] 明文变换(BitArray plain)
        {
        ...
        private BitArray 加密右边32变48(BitArray R32)
        {
            BitArray ret = new BitArray(48);
        ...
        private BitArray[] 右边32变48之后和K做异或(BitArray e3248, BitArray key48)
        {
        ...
        private BitArray 做S变换(BitArray[] B)
        {
        ...
        private BitArray S变换后的变换与异或(BitArray B32, BitArray L32)
        {
        ...
        private BitArray Encrypt(BitArray plain, BitArray key, bool 加密)
        {
